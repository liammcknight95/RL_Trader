...
def intraday_vol_ret(px_ts, span=100):
    ...
    assert isinstance(
        px_ts.index, pd.DatetimeIndex
    ), "px series must have a datetime index"

    df = px_ts.index.searchsorted(
        px_ts.index - pd.Timedelta(days=1)
    )  # returns a scalar array of insertion point
    df = df[df > 0]  # remove inserion points of 0
    df = pd.Series(
        px_ts.index[df - 1], index=px_ts.index[px_ts.shape[0] - df.shape[0] :]
    )  # -1 to rebase to 0, index is a shifted version
    ret = (
        px_ts.loc[df.index] / px_ts.loc[df.values].values - 1
    )  # "today" / yesterday - 1 -> 1d rolling returns
    vol = ret.ewm(
        span=span
    ).std()  # exponential weighted std. Specify decay in terms of span
...
def data_cleaning(df_px, df_trade):
    ...
    df_trade["Datetime"] = pd.to_datetime(
        df_trade["Datetime"], format="%Y-%m-%d %H:%M:%S"
    )
...
def data_standardization(df_data, norm_type, roll, stdz_depth=1):
    ...
    depth_size_dyn_stdz = standardize(
        df_data[std_depth_size_cols], stdz_depth, norm_type, roll
    )

    trd_size_dyn_stdz = standardize(
        df_data[std_trade_size_cols], stdz_depth, norm_type, roll
    )

    depth_level_dyn_stdz = standardize(
        df_data[std_depth_level_cols], stdz_depth, norm_type, roll
    )

    trade_number_dyn_stdz = standardize(
...
def train_test_split(df, pctg_split=0.7, stdz_depth=1):
    ...
    train_test_split = int(
        (df.shape[0] / stdz_depth) * pctg_split
    )  # slice reference for train and test
...
def import_data(
    ...
    depth=100,
):  # , norm_type='dyn_z_score', roll=1440, stdz_depth=1):
    """
    ...
    # import px
    results_px = get_lob_data(
        pair, date_start, date_end, frequency, depth, s3_download=s3_download
    )
    # print(results_px)
...
def import_px_data(frequency, pair, date_start, date_end, lob_depth, norm_type, roll):
    ...
    standardized_train_file = (
        f"{resampled_data_folder}/{pair}/TRAIN--{norm_type}-{roll}--{quotes_file_name}"
    )
    standardized_test_file = (
        f"{resampled_data_folder}/{pair}/TEST--{norm_type}-{roll}--{quotes_file_name}"
    )
    ...
    if os.path.isfile(
        ...
        quotes_data_input_list = get_lob_data(
            pair, date_start, date_end, frequency, lob_depth
        )
        ...
        data = pd.concat([trades_data_input_pd, quotes_data_input_pd]).sort_values(
            by=["Datetime", "Level"]
        )
...
def standardized_data_cache(
    ...
    # Train test split
    train_test_split = int(
        (data.shape[0] / stdz_depth) * 0.7
    )  # slice reference for train and test
    train_timestamps = data["Datetime"].unique()[:train_test_split]
    test_timestamps = data["Datetime"].unique()[train_test_split:]
    train_cached_data = data[data["Datetime"].isin(train_timestamps)].set_index(
        ["Datetime", "Level"]
    )
    test_cached_data = data[data["Datetime"].isin(test_timestamps)].set_index(
        ["Datetime", "Level"]
    )

    print(
        f"All data shape: {data.shape} - Train dataset shape: {train_cached_data.shape} - Test dataset shape: {test_cached_data.shape}"
    )

    roll_shift = (
        roll  # rolling period for dyn z score - + 1 from shift in standardize()
    )

    # Training
    # custom rolling standardization for px and size separately
    train_dyn_prices = standardize(
        train_cached_data[["Ask_Price", "Bid_Price"]], stdz_depth, "dyn_z_score", roll
    )
    train_dyn_volumes = standardize(
        train_cached_data[["Ask_Size", "Bid_Size"]], stdz_depth, "dyn_z_score", roll
    )
    train_dyn_df = pd.concat(
        [train_dyn_prices, train_dyn_volumes], axis=1
    ).reset_index()  # concat along row index #1
    print(f"Saving {standardized_train_file}")
    train_dyn_df.to_csv(
        standardized_train_file, compression="gzip"
    )  # save standardized data to csv
    # train_dyn_df.reset_index(inplace=True)

    top_ob_train = train_cached_data[train_cached_data.index.get_level_values(1) == 0][
        roll_shift:
    ]  # 3
    top_ob_train["Mid_Price"] = (
        top_ob_train["Ask_Price"] + top_ob_train["Bid_Price"]
    ) / 2
    top_ob_train["Spread"] = (
        top_ob_train["Ask_Price"] - top_ob_train["Bid_Price"]
    ) / top_ob_train["Mid_Price"]
    top_ob_train[
        "merge_index"
    ] = top_ob_train.reset_index().index.values  # useful for merging later
    ...
    # Test
    # custom rolling standardization for px and size separately
    test_dyn_prices = standardize(
        test_cached_data[["Ask_Price", "Bid_Price"]], stdz_depth, "dyn_z_score", roll
    )
    test_dyn_volumes = standardize(
        test_cached_data[["Ask_Size", "Bid_Size"]], stdz_depth, "dyn_z_score", roll
    )
    test_dyn_df = pd.concat(
        [test_dyn_prices, test_dyn_volumes], axis=1
    ).reset_index()  # concat along row index #2
    print(f"Saving {standardized_test_file}")
    test_dyn_df.to_csv(
        standardized_test_file, compression="gzip"
    )  # save standardized data to csv
    # test_dyn_df.reset_index(inplace=True)

    top_ob_test = test_cached_data[test_cached_data.index.get_level_values(1) == 0][
        roll_shift:
    ]  # 4
    top_ob_test["Mid_Price"] = (top_ob_test["Ask_Price"] + top_ob_test["Bid_Price"]) / 2
    top_ob_test["Spread"] = (
        top_ob_test["Ask_Price"] - top_ob_test["Bid_Price"]
    ) / top_ob_test["Mid_Price"]
    top_ob_test[
        "merge_index"
    ] = top_ob_test.reset_index().index.values  # useful for merging later
...
def standardize(ts, stdz_depth, norm_type="z_score", roll=0):
    ...
    # dynamic can't accomodate multi columns normalization yet
        ...
        if ts_shape > 1:
            ...
            print(
                f"rolling window = {roll * stdz_depth * ts_shape}, calculate as roll: {roll} * levels: {stdz_depth} * shape[1]: {ts_shape}"
            )

            ts_dyn_z = (
                ts_stacked
                - ts_stacked.rolling(roll * stdz_depth * ts_shape)
                .mean()
                .shift((stdz_depth * ts_shape) + 1)
            ) / ts_stacked.rolling(roll * stdz_depth * ts_shape).std(ddof=0).shift(
                (stdz_depth * ts_shape) + 1
            )

            if stdz_depth > 1:
                norm_df = ts_dyn_z.reset_index().pivot_table(
                    index=["Datetime", "Level"], columns="level_2", values=0
                )  # , dropna=True)
...
def get_lob_download_only(
    pair, date_start, date_end, dnwld_type, configuration=configuration
):
    """lightweight version of get_lob_data only for data download
    dnwld_type: lob or trade
    """

    raw_data_folder = configuration["folders"][
        f"raw_{dnwld_type}_data"
    ]  # raw data local destination folder
    ...
    while date_to_process <= date_end:
        try:
            ...
            data_bucket = s3_resource.Bucket(
                configuration["buckets"][f"{dnwld_type}_data"]
            )
            ...
            if dnwld_type == "lob":
                ...
                if not os.path.isdir(raw_file_folder):
                    ...
                    shutil.move(
                        f"{raw_data_folder}/tmp/{pair}/{day_folder}", raw_file_folder
                    )
                else:
                ...
                raw_file_name = (
                    f'{pair}-{datetime.strftime(date_to_process, "%Y%m%d")}.csv.gz'
                )
                raw_file_path = f"{raw_data_folder}/{pair}/{raw_file_name}"

                if not os.path.isfile(raw_file_path):

                    data_bucket.download_file(
                        f"{pair}/{raw_file_name}", f"{raw_file_path}"
                    )
...
def ingest_single_day(
    ...
    assert frequency >= timedelta(
        seconds=1
    ), "Frequency must be equal to or greater than 1 second"
    ...
    os.makedirs(
        f"{resampled_data_folder}/{pair}/{lob_depth}_levels/{freq}", exist_ok=True
    )
    ...
    if os.path.isfile(resampled_file_path):
        ...
        raw_data_frame = (
            raw_data_frame.reindex(idx).ffill().fillna(method="bfill")
    ...
    day_data = pd.DataFrame(
        [y for x in processed_data for y in x],  # flatten the list of lists structure
        columns=[
            "Ask_Price",
            "Ask_Size",
            "Bid_Price",
...
def get_lob_data(
    ...
    while date_to_process <= date_end:
        ...
        if os.path.isfile(resampled_file_path):
            ...
            if os.path.isfile(original_file_name):
                ...
                if s3_download:
                    ...
                    if not os.path.isdir(f"{raw_data_folder}/{pair}/{day_folder}"):
                        ...
                        download_s3_folder(lob_data_bucket, day_folder, keys)
                        shutil.move(
                            f"{raw_data_folder}/tmp/{pair}/{day_folder}",
                    ...
                    day_data = pd.DataFrame(
                        [
                            y for x in processed_data for y in x
                    ...
                    print(
                        f"No file found, s3 download set to False, skipping {date_to_process}"
                    )
                    date_to_process += timedelta(
                        days=1
                    )  # the most nested folder is a day of the month
                    continue
            ...
            day_data["Prevailing_Mid"] = day_data.groupby("Datetime")[
                "Mid_Price"
            ].transform("first")

            # cumulative depth size
            day_data["Bid_Cum_Size"] = day_data.groupby("Datetime")[
                "Bid_Size"
            ].transform(np.cumsum)
            day_data["Ask_Cum_Size"] = day_data.groupby("Datetime")[
                "Ask_Size"
            ].transform(np.cumsum)

            # spread against prevailing mid
            day_data["Bid_Spread"] = (
                (day_data["Bid_Price"] - day_data["Prevailing_Mid"])
                / day_data["Prevailing_Mid"]
                * -1
            )
            day_data["Ask_Spread"] = (
                day_data["Ask_Price"] - day_data["Prevailing_Mid"]
            ) / day_data["Prevailing_Mid"]

            # resample dataframe to the wanted frequency
            day_data_grp = (
                day_data.groupby([pd.Grouper(key="Datetime", freq=freq), "Level"])
                .agg(
                    {
                        "Bid_Spread": np.mean,
                        "Ask_Spread": np.mean,
            ...
            def get_depth(df, side, target_sprd):
                ...
                if max_lvl == 99:
                    print(
                        f"timestamps where level has been maxed out: {df_grouped[df_grouped[f'{side}_Level_{tgt_sprd_bps}bps']==99].index}"
                    )
            ...
            df_px_lv0 = day_data_grp[day_data_grp["Level"] == 0][
                ["Datetime", "Ask_Price", "Bid_Price", "Mid_Price"]
            ].set_index("Datetime")
            df_px_final = pd.merge(
                df_px_lv0, df_depth, left_index=True, right_index=True, how="left"
            ).reset_index()
...
def download_s3_folder(lob_data_bucket, day_folder, keys):
    ...
    with futures.ThreadPoolExecutor(max_workers=100) as executor:
        future_to_key = {
            executor.submit(
                download_S3_object, lob_data_bucket, key, f"{raw_data_folder}/tmp"
            ): key
            for key in keys
        }
...
def load_lob_json(json_string):
    ...
    try:
        ...
        print(
            f"Malformed JSON {e.msg} in file at position {e.pos} - {json_string[e.pos:e.pos+10]}"
        )
        ...
        if e.msg == "Expecting value":
            ...
            if prev_snapshot_start == 0:
                ...
                fixed_json_string = (
                    json_string[:prev_snapshot_end] + json_string[e.pos :]
                )
    ...
    while date_to_process <= date_end:
        ...
        if os.path.isfile(resampled_file_path):
            ...
            if not os.path.isfile(raw_file_path):
                ...
                trade_data_bucket.download_file(
                    f"{pair}/{raw_file_name}", f"{raw_file_path}"
                )
                print(f"Downloaded {raw_file_name} from S3")
            ...
            df_trades_piv = df_trades_grp.pivot(
                values=["amount", "av_price", "wav_price", "unique_orders", "clips"],
                columns="type",
                index="Datetime",
            ).reset_index()
...
def cnn_data_reshaping(X, Y, T):
    ...
    dataX = dataX.reshape(
        dataX.shape + (1,)
    )  # no need to add the extra dimension for 1d conv

...
from bot_db_class import BotDB
...
class TradingBot(BotDB):
    ...
    def __init__(self, exchange_subaccount, pair, strategy, frequency, sl_type, sl_pctg, owned_ccy_size, opening_position, container_id, container_name, database_setup, sandbox=True, **params):
        ...
        try:
            ...
            self.get_db_config(database_setup)
            # if database_setup == 'local': database_setup = 'local_docker'
            # self.db_config_parameters = config.pg_db_configuration(location=database_setup) ## MOVED
    ...
    def get_ob_and_sizing(self):
        ...
        # need both legs of a trade to recalibrate owned_ccy_size
    ...
    def create_new_order(self):
        ...
        # set limit order price accounting for order book ticksize
        try:
            self.order_price = round(self.top_ask_px * 1.001, tick_size_decimals[self.pair])
        except Exception as e:
            if type(e).__name__ == 'KeyError':
                self.logger.warning(f"!! Setting limit order price without tick size specification - check tick_size_decimals in bot_config.py")
    ...
    def orders_error_handling(self, e):
        ''' Handle orders related exception.
            if exception is raised, it's then captured by the run_bot exception block
        ...
        if type(e).__name__ == 'BadSymbol':
            ...
            # insufficient funds - cauase bot to terminate
    ...
    def _check_buy_sell_signals(self):
        ...
        if not self.in_position:
            ...
            if self.sell_order:
                ...
                # business as usual
                if self.bars_df.loc[previous_period][f'{self.strategy}_signal']==1 and self.bars_df.loc[previous_period]['timestamp']>self.signal_time:
            ...
            if self.order_executed_check(self.buy_order['id']):
                ...
                if self.bars_df.loc[previous_period][f'{self.strategy}_signal']==0 or self.current_price <= self.sl_price:
                ...
                if self.bars_df.loc[previous_period][f'{self.strategy}_signal']>0 and self.current_price > self.sl_price:
